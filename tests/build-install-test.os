...
Процедура ТестДолжен_ОчиститьКаталогОтСтарыхСкриптовПриОбновленииПакета() Экспорт
	...
	КаталогСборкиПервойВерсии = юТест.ИмяВременногоФайла();

	СоздатьКаталог(КаталогСборкиПервойВерсии);
	...
	КаталогСборкиВторойВерсии = юТест.ИмяВременногоФайла();

	СоздатьКаталог(КаталогСборкиВторойВерсии);
	Сборщик.СобратьПакет(
	...
	ПутьУстановкиФайловТестовогоПакета = ОбъединитьПути(
		Константы.ЛокальныйКаталогУстановкиПакетов, "test","folder","src.os");

	ФайлИзСтарогоПакета = Новый Файл(ПутьУстановкиФайловТестовогоПакета);

	Утверждения.ПроверитьЛожь(ФайлИзСтарогоПакета.Существует(), "Файл src.os не должен существовать, так как удален в новой версии пакета");
