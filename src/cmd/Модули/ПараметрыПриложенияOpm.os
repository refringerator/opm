...
Процедура НастроитьOpm() Экспорт
	...
	НастройкиПроксиЕсть = ПолучитьЗначение(НастройкиOpmИзФайлов,"Прокси", ЛОжь);
		
	Если НастройкиПроксиЕсть Тогда

		НастройкиПрокси = НастройкиOpmИзФайлов.Прокси;

		Сервер = ПолучитьЗначение(НастройкиПрокси, "Сервер", "");
		Порт = ПолучитьЗначение(НастройкиПрокси, "Порт", 80);
	...
	Если Не СервераПакетов = Неопределено Тогда
		...
		Для каждого ДополнительныйСервер Из СервераПакетов Цикл
			...
			Порт = ПолучитьЗначение(ДополнительныйСервер, "Порт", 80);
			...
			Приоритет = ПолучитьЗначение(ДополнительныйСервер, "Приоритет", 0);
...
Процедура ЗаполнитьНастройкиИзПараметров(Знач ПараметрКоманды) Экспорт
	...
	Если НЕ ПараметрКоманды["-proxyusedefault"] = Неопределено Тогда
		...
		НастройкиOpm.УстановитьСистемныеНастройкиПроксиСервера();
	...
	НастройкиПрокси = НастройкиOpm.ПолучитьНастройки();
	...
	ИспользованиеПрокси = ЗначениеЗаполнено(ПроксиСервер) ИЛИ ЗначениеЗаполнено(ПараметрКоманды["-proxyusedefault"]);
	НастройкиOpm.УстановитьИспользованиеПрокси(ИспользованиеПрокси);
